...
> **赞助页已关闭**：`siteConfig.pages.sponsor = false`，访问 `/sponsor/` 直接 404，文中涉及赞助的描述均已失效。
> **加载页已重做**：见文末「2026-07-31 更新日志」，现为纯 CSS 打字机状态短语 + 均衡器动效，不含站名 / 人名。
> 本文中凡涉及「留言板」「评论系统」「赞助」的描述，均为模板原始能力说明，不代表本站当前已启用。
...
| Pagefind | 全文搜索 |
| Waline / Twikoo / Giscus | 评论系统（可选·**本站已停用**，见顶部当前状态） |
| Live2D | 看板娘 |
...
const avatarSrc = "/gallery/测试/QQ.png";
...
```typescript
{
  ...
  // 导航栏
  navbar: {
    logo: { type: "image", value: "/gallery/测试/QQ.png" },
    title: "非洲和尚的个人博客",
    widthFull: false,
  },
...
| Waline / Twikoo / Giscus | 评论（**本站已停用**，见顶部当前状态） | `commentConfig.ts` |
...
**4. 置顶收敛**
- 仅本文（`博客功能全解析.md`）保持 `pinned: true`，其余文章均已移除
...
### 2026-07-31 加载页改版 · 赞助关闭 · 代码体检 · 流畅度优化

围绕「去原作者痕迹 + 流畅度 + 文档同步」做了一次集中维护，全部已提交。

**一、加载页重做（三版迭代，最终落地）**
- 移除原作者遗留的 `feibi-loading.webp` 动图，彻底与原作者素材脱钩
- 终版形态：`PageLoader.astro` 改为**纯 CSS 转圈（品牌粉 `#D44E7D`）+ 打字机状态短语 + 5 条均衡器动效**，由内联脚本驱动、**零依赖**
- 打字机循环通用状态短语（如「正在唤醒博客引擎 / 加载主题与配色 / 同步最新文章 / 校准视觉特效」），粉色光标闪烁；**不含站名 / 人名**（用户明确不想要）
- 保留契约：`page-loader-controller.js` 不动；移动端 `<=768px` 隐藏、`prefers-reduced-motion` 关闭动画均保留

**二、赞助页关闭**
- `siteConfig.pages.sponsor = false`，`/sponsor/` 访问即 404
- `sponsorConfig.ts` 清空 `methods` / `sponsors`，仅留中性文案

**三、原作者痕迹清除**
- `AboutCanvas` 移除弹球图加载（`about.webp` 不再使用）
- `about.md` 署名「Mmz明崽」改为「Astro 生态自主搭建」
- `footerConfig.ts` 移除 Firefly / CuteLeaf 链接，仅留 Astro

**四、代码体检**
- 跑 `astro check`，修复全部 13 个类型错误（commit `1efbc59`），构建期类型校验 0 error
- 删除 20 篇非置顶文章及其孤儿配图，清空空壳目录与 `public` 僵尸副本

**五、访问流畅度优化**
- **首屏 LCP**：首页 hero 背景图加 `<link rel="preload" as="image" fetchpriority="high">`（桌面 / 移动分别预加载），`<img>` 加 `fetchpriority="high" decoding="async"`
- **静态资源缓存**：`vercel.json` 对 `/_astro/*` 设 `public, max-age=31536000, immutable`（哈希资源长期缓存）
- **SPA 导航**：`@swup/astro` 开启 `preload: { hover: true, visible: true }`，悬停 / 进入视口即预取目标页，切换近乎无等待
- **字体**：自托管 `AaZongYiYuan` 并 `<link rel="preload">`，无 CDN FOUC

**六、友链双向验证（仅分析，未实装）**
- 参考 2x.nz/friends：访客提交站点 → 系统检测对方站是否回链本站 → 通过后友链才显示
- 静态站可行方案：构建期校验脚本（GitHub Action）+ `friendsConfig.applyLink` 申请表单 + `verified` 字段驱动卡片显隐；属后续功能
