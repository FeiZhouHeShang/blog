...
- `commentConfig.ts` - 评论系统配置（Waline / Twikoo / Giscus / Artalk / Disqus）。**当前 `type: "none"` 已禁用**：原 Waline 是源码原作者私人服务器，评论/留言板功能已移除，保留接口以备自行部署。
...
- `friendsConfig.ts` - 友链页配置（导出 `friendsPageConfig`；友链列表已迁至后台内容集合 `src/content/friends/`）
- `galleryConfig.ts` - 相册配置
